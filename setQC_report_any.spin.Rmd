...
```{r check_sample_info,echo=F,warning=F,cache=F,message=F}
if(has_sample_table) {
...
```{r fastq_module,echo=F,message=F,warning=F}
...
```{r fastq_screen,echo=F,message=F,warning=F}
tlist <- list()
tlist[[1]]<- plotSource()
tagList(tlist)
```
...
```{r tss_enrich_plot,echo =F, warning=F}
#require(evaluate)
...
```{r avg_tss,echo=F,warning=F,message=F}
# read data
...
```{r max_tss,echo=F,warning=F,message=F}
# bargraph for the
...
```{r insert_size,echo =F,warning=F}
...
```{r libqc_table,echo=F,message=F}
...
---
title: "setQC_report_any.R"
author: "zhc268"
date: "Tue Nov  7 11:15:32 2017"
---
